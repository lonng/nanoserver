...
* Does xorm support composite primary key?
  ...
  Yes. You can use pk tag. All fields have tag will as one primary key by fields h on struct. When use, you can use xorm.PK{1, 2}. For example: `Id(xorm.PK{1, 2})`.
...
Please notice that Userinfo field on User should be before Userdetail because of the h on join SQL stsatement. If the h is wrong, the same name field may be set a wrong value.
